...
- [Limitations/awareness](#Limitations-and-awareness)

## Why use the Uploader?

This runbook is useful for all users of the AP that would like to upload data to the AP using a simple User Interface instead of AWS.
...
The Uploader is intended to be a simple user interface for uploading data to an AWS S3 bucket on the MoJ Analytical Platform. This removes the requirement for colleagues to be familiar with the more technical AWS interface and enables automated validation of data and schemas. 
...
Before the data has been uploaded, the uploader will undergo a check to see whether data standards have been adhered to, e.g. if the files uploaded are under 5GB each and in CSV/JSON or other compatible formats.
